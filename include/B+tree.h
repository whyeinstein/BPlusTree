...
T InterNode<T>::Split(InterNode<T>* pNode, T key)  //key是新插入的值，pNode是分裂结点
{
    ...
    if ((key > this->GetKeyValue(ORDER-1)) && (key < this->GetKeyValue(ORDER )))//？？？[0 1 key 2 3]？？？
    {
        int x;
        ...
        {         
            ...
            int m=0;
            for(m=0;(m<(pNode->GetCount()))&&(this->GetKeyValue(i) > pNode->GetKeyValue(m));m++);   
...
bool InterNode<T>::Borrow(InterNode<T>* pNode) {// 定义成InterNode而非Node？？？
    ...
    if (pNode->GetKeyValue(0) < this->GetKeyValue(0))
    {
        ...
        pNode->m_KeyValues.erase(pNode->m_KeyValues.begin()+pNode->GetCount()-1);
    }
    else    // 兄弟结点在本结点右边
    {
        // 赋值
...
void InterNode<T>::Delete(T key)
{
    int i;
    //找到比当前key值大的位置i
    for (i = 0; (key >= this->GetKeyValue(i)) && (i < this->GetCount()); i++);
...
T LeafNode<T>::Split(LeafNode<T>* pNode)
{
    ...
    int j = 0;
    for (int i = this->GetCount()-1; i >= (this->GetCount()+1)/2; i--) // 注意从哪开始分裂？？？
    {
        j++;
        //将原i位置上的元素删除并添加到新节点中
...
template<typename foo>
bool BPlusTree<foo>::InsertInterNode(InterNode<foo> *pNode, foo key, Node<foo> *rightChild){
...
bool BPlusTree<foo>::DelInterNode(InterNode<foo>* pNode, foo key){
    ...
    {
        for (int i = 0; (key >= pFather->GetKeyValue(i)) && (i < pFather->GetCount()); i++) //？？？=还是<=
        {
            // 如果删除的是父结点的键值，需要更改该键
            if (pFather->GetKeyValue(i) == key)
            {
                pFather->ReplaceKeyValue(i, pNode->GetKeyValue(0));    // 更改为孩子结点新的第一个元素
            }
        }
        return true;
    }
...
void BPlusTree<foo>::RangeQuery(foo minKey,foo maxKey) {
    ...
    while(pNode!=NULL) {
        for(int i=0;i<pNode->GetCount();i++) {
            if (pNode->GetKeyValue(i)>=minKey) {
                if (pNode->GetKeyValue(i)<=maxKey) {
