...
*/
template <typename T>
T InterNode<T>::Split(InterNode<T>* pNode, T key, int MAX_KEYNUM,
                      int ORDER)  // key是新插入的值，pNode是分裂结点
{
  int i = 0, j = 0;
...
bool BPlusTree<foo>::DelInterNode(InterNode<foo>* pNode, foo key) {
  ...
  if (pNode->GetCount() >= ORDER) {
    for (int i = 0;
         (key >= pFather->GetKeyValue(i)) && (i < pFather->GetCount());
         i++)  //？？？=还是<=
    {
      // 如果删除的是父结点的键值，需要更改该键
...
void BPlusTree<foo>::RangeQuery(foo minKey, foo maxKey) {
  ...
  while (sNode->GetNodeType() != 2) {
    int i = BinALG(sNode, sNode->GetCount() - 1, minKey);
    sNode = sNode->GetChild(i);
  }
  int i = BinALG(sNode, sNode->GetCount() - 1, minKey);
  int j = i;
  if (sNode->GetKeyValue(i) != minKey) {
